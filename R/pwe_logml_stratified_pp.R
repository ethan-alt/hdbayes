...
#' likelihood of a PWE model under the stratified power prior (PP).
#'
...
#'
#' @include pwe_stratified_pp_lognc.R
#'
#' @export
#'
#' @param post.samples      output from [pwe.stratified.pp()] giving posterior samples of a PWE model under the stratified
...
#'   }
#' }
pwe.logml.stratified.pp = function(
    post.samples,
    bridge.args       = NULL,
    iter_warmup       = 1000,
    iter_sampling     = 1000,
  ...
  res.all = pwe.stratified.pp.lognc(
  ...
  pwe_stratified_pp_prior = instantiate::stan_package_model(
    name = "pwe_stratified_pp_prior",
    package = "hdbayes"
  )

  fit = pwe_stratified_pp_prior$sample(data = stan.data,
  ...
  res.hist = pwe.stratified.pp.lognc(
