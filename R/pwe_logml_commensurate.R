...
#' likelihood of a PWE model under the commensurate prior (CP).
#'
...
#'
#' @include pwe_commensurate_lognc.R
#'
#' @export
#'
#' @param post.samples      output from [pwe.commensurate()] giving posterior samples of a PWE model under the commensurate
...
#'   }
#' }
pwe.logml.commensurate = function(
    post.samples,
    bridge.args       = NULL,
    iter_warmup       = 1000,
    iter_sampling     = 1000,
  ...
  res.all = pwe.commensurate.lognc(
  ...
  ## sample from CP
  pwe_commensurate_prior = instantiate::stan_package_model(
    name = "pwe_commensurate_prior",
    package = "hdbayes"
  )

  fit = pwe_commensurate_prior$sample(data = stan.data,
  ...
  res.hist = pwe.commensurate.lognc(
