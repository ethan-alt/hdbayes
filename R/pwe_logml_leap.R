...
#' likelihood of a PWE model under the latent exchangeability prior (LEAP).
#'
...
#' data sets.
#'
#' @include pwe_leap_lognc.R
#'
#' @export
#'
#' @param post.samples      output from [pwe.leap()] giving posterior samples of a PWE model under the latent exchangeability
...
pwe.logml.leap = function(
  ...
  ## sample from LEAP
  pwe_leap_prior   = instantiate::stan_package_model(
    name = "pwe_leap_prior",
    package = "hdbayes"
  )

  fit = pwe_leap_prior$sample(data = stan.data,
