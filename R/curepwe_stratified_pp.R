...
#' The CurePWE model assumes that a fraction \eqn{\pi} of the population is "cured", while the remaining \eqn{1 - \pi}
#' are susceptible to the event of interest. The survival function for the entire population is given by:
...
#' baseline hazard parameters with half-normal priors. Additionally, a normal prior is specified for the logit of
#' the cure fraction within each stratum.
#'
...
#'   }
#' }
curepwe.stratified.pp = function(
    formula,
    data.list,
    strata.list,
    breaks,
  ...
  ## Default prior on logit(cure fraction) within each stratum is N(0, 3^2)
  ...
  curepwe_stratified_pp = instantiate::stan_package_model(
    name = "curepwe_stratified_pp",
    package = "hdbayes"
  )

  ## fit model in cmdstanr
  fit = curepwe_stratified_pp$sample(data = standat,
  ...
  attr(x = d, which = 'model') = "curepwe_stratified_pp"
  return(d)
}
