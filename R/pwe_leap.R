...
#' with a piecewise constant baseline hazard) using the latent exchangeability prior (LEAP) by Alt et al. (2024)
...
pwe.leap = function(
  ...
  pwe_leap = instantiate::stan_package_model(
    name = "pwe_leap",
    package = "hdbayes"
  )

  ## fit model in cmdstanr
  fit = pwe_leap$sample(data = standat,
