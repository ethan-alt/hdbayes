#' Log marginal likelihood of a standard cure rate (CurePWE) model under the meta-analytic predictive (MAP) prior
#'
#' @description Uses Markov chain Monte Carlo (MCMC) and bridge sampling to estimate the logarithm of the marginal
#' likelihood of a CurePWE model under the meta-analytic predictive (MAP) prior. The MAP prior is equivalent to the prior
...
#' samples are then used to compute the logarithm of the normalizing constant of the MAP prior using only historical
#' data set.
#'
#' @include curepwe_bhm_lognc.R
#'
#' @export
#'
#' @param post.samples      output from [curepwe.bhm()] giving posterior samples of a CurePWE model under the Bayesian
#'                          hierarchical model (BHM), with an attribute called 'data' which includes the list of variables
...
curepwe.logml.map = function(
  ...
  ## computing log normalizing constant for the prior induced by BHM using all data sets
  res.all = curepwe.bhm.lognc(
    post.samples   = post.samples,
    is.prior       = FALSE,
    bridge.args    = bridge.args
  )

  ## sample from the prior induced by BHM
  curepwe_bhm_prior = instantiate::stan_package_model(
    name = "curepwe_bhm_prior",
    package = "hdbayes"
  )

  fit = curepwe_bhm_prior$sample(data = stan.data,
  ...
  ## compute log normalizing constant for the prior induced by BHM using historical data set
  res.hist = curepwe.bhm.lognc(
