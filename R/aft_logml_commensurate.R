...
#' likelihood of an AFT model under the commensurate prior (CP).
#'
...
#'
#' @include aft_commensurate_lognc.R
#'
#' @export
#'
#' @param post.samples      output from [aft.commensurate()] giving posterior samples of an AFT model under the commensurate
...
#'   }
#' }
aft.logml.commensurate = function(
    post.samples,
    bridge.args       = NULL,
    iter_warmup       = 1000,
    iter_sampling     = 1000,
  ...
  res.all = aft.commensurate.lognc(
  ...
  ## sample from CP
  aft_commensurate_prior = instantiate::stan_package_model(
    name = "aft_commensurate_prior",
    package = "hdbayes"
  )

  fit = aft_commensurate_prior$sample(data = stan.data,
  ...
  res.hist = aft.commensurate.lognc(
