...
#' data sets.
#'
#' @include aft_leap_lognc.R
#'
#' @export
#'
#' @param post.samples      output from [aft.leap()] giving posterior samples of an AFT model under the latent exchangeability
...
aft.logml.leap = function(
  ...
  ## sample from LEAP
  aft_leap_prior   = instantiate::stan_package_model(
    name = "aft_leap_prior",
    package = "hdbayes"
  )

  fit = aft_leap_prior$sample(data = stan.data,
