...
#'
#' @include aft_stratified_pp_lognc.R
#'
#' @export
#'
#' @param post.samples      output from [aft.stratified.pp()] giving posterior samples of an AFT model under the stratified
...
#'   }
#' }
aft.logml.stratified.pp = function(
    post.samples,
    bridge.args       = NULL,
    iter_warmup       = 1000,
    iter_sampling     = 1000,
  ...
  res.all = aft.stratified.pp.lognc(
  ...
  aft_stratified_pp_prior = instantiate::stan_package_model(
    name = "aft_stratified_pp_prior",
    package = "hdbayes"
  )

  fit = aft_stratified_pp_prior$sample(data = stan.data,
  ...
  res.hist = aft.stratified.pp.lognc(
