...
#' @noRd
#'
#' @param post.samples      posterior samples of a CurePWE model under the Bayesian hierarchical model (BHM) or samples from
#'                          the prior induced by the BHM, with an attribute called 'data' which includes the list of variables
...
curepwe.bhm.lognc = function(
  ...
  if( is.prior ){
    ...
    oldnames = c(oldnames, paste0('beta_raw[', 1:p, ']'), paste0('beta0_raw[', 1:p, ']'),
                 "logit_p_cured", "logit_p_cured0")
  }
  ...
  if( is.prior ){
    lb           = c(rep(0, J), rep(-Inf, 1+2*p), rep(0, p))
  }else{
    lb           = c(rep(0, 2*J), rep(-Inf, 2+3*p), rep(0, p))
  }
