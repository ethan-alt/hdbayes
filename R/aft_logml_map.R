...
#' likelihood of an AFT model under the meta-analytic predictive (MAP) prior. The MAP prior is equivalent to the prior
...
#' @include aft_bhm_lognc.R
#'
#' @export
#'
#' @param post.samples      output from [aft.bhm()] giving posterior samples of an AFT model under the Bayesian hierarchical
...
aft.logml.map = function(
  ...
  aft_bhm_prior    = instantiate::stan_package_model(
    name = "aft_bhm_prior",
    package = "hdbayes"
  )

  fit = aft_bhm_prior$sample(data = stan.data,
