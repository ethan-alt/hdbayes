...
#' likelihood of a PWE model under the meta-analytic predictive (MAP) prior. The MAP prior is equivalent to the prior
...
#' data set.
#'
#' @include pwe_bhm_lognc.R
#'
#' @export
#'
#' @param post.samples      output from [pwe.bhm()] giving posterior samples of a PWE model under the Bayesian hierarchical
...
pwe.logml.map = function(
  ...
  pwe_bhm_prior    = instantiate::stan_package_model(
    name = "pwe_bhm_prior",
    package = "hdbayes"
  )

  fit = pwe_bhm_prior$sample(data = stan.data,
