#' Log marginal likelihood of a standard cure rate (CurePWE) model under latent exchangeability prior (LEAP)
#'
#' @description Uses Markov chain Monte Carlo (MCMC) and bridge sampling to estimate the logarithm of the marginal
#' likelihood of a CurePWE model under the latent exchangeability prior (LEAP).
#'
...
#' data sets.
#'
#' @include curepwe_leap_lognc.R
#'
#' @export
#'
#' @param post.samples      output from [curepwe.leap()] giving posterior samples of a CurePWE model under the latent
#'                          exchangeability prior (LEAP), with an attribute called 'data' which includes the list of variables
...
curepwe.logml.leap = function(
  ...
  ## computing log normalizing constant for LEAP using all data sets
  res.all = curepwe.leap.lognc(
  ...
  ## sample from LEAP
  curepwe_leap_prior = instantiate::stan_package_model(
    name = "curepwe_leap_prior",
    package = "hdbayes"
  )

  fit = curepwe_leap_prior$sample(data = stan.data,
