...
denoted by $D_{0h} = \{ (y_{0hi}, \bm{x}_{0hi}), i = 1, \ldots, n_{0h} \}$, where 
...
\end{align*}
$$
where $\beta = (\beta_1, \ldots, \beta_p)'$, $\beta_{0} = (\beta_{01}, \ldots, \beta_{0p})'$, and the $\tau_j$'s are elicited by the user. The defaults in `hdbayes` are
...
The default in `hdbayes` is a (noninformative) normal initial prior on $\beta$:
$$
...
when $\alpha_{0h} = 1$ and $\gamma_{0h} = 1$, the prior on $a_{0h}$ is a $U(0,1)$
prior.
