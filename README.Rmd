...
\end{align*}
$$
where $p_I(\beta | D, D_0) = L(\beta | D) \pi_I(\beta | D_0) / Z_I(D, D_0)$ is the posterior density under the informative prior, which is the prior induced by the BHM (referred to as the meta-analytic predictive (MAP) prior), $p_V(\beta | D) = L(\beta | D) \pi_V(\beta) / Z_V(D)$ is the posterior density under the vague prior, and
$$
